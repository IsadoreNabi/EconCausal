...
2. **Compensación con un filtro posterior**: El ECM con $\lambda$ significativamente negativo, exigido en la misma ventana de entrenamiento, filtra falsos positivos
...
donde `folds_proceed` cuenta los folds cuya ventana de entrenamiento pasa todos los filtros econométricos: ambas series integradas de orden uno, cointegración según la regla elegida y un coeficiente de corrección de error significativamente negativo. El soporte mide, por lo tanto, cuán persistentemente se cumplen esas condiciones de admisibilidad a través de las ventanas, no la precisión del pronóstico; las métricas de pronóstico de las ventanas de prueba se reportan por separado y no entran en `folds_proceed`.
...
2. **Control de BLAS**: Si `RhpcBLASctl` está instalado, BLAS y OpenMP se fijan en un hilo en la sesión de R que llama y se restauran al salir; las sesiones trabajadoras conservan su propia configuración de hilos

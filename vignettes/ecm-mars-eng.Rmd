...
2. **Compensation with a subsequent filter**: The ECM with a significantly negative $\lambda$, required in the same training window, filters false positives
...
where `folds_proceed` counts the folds whose training window passes all econometric filters: both series integrated of order one, cointegration under the chosen rule, and a significantly negative error-correction coefficient. Support therefore measures how persistently these admissibility conditions hold across windows, not forecast accuracy; the forecast metrics of the test windows are reported separately and do not enter `folds_proceed`.
...
2. **BLAS control**: when `RhpcBLASctl` is installed, BLAS and OpenMP are set to one thread in the calling R session and restored on exit; the worker sessions keep their own thread settings
